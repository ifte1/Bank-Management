    ...
    while(run_choice!=0.0):
        ...
        if run_choice == 2:
            ...
            Stream(names).source() \
                ...
                .map(lambda n : len(n)) \
                .print()
            print(f'found names with 8 letters.')
        ...
        if run_choice == 5 or run_choice == 5.1:
            # Challenge 5, Pipeline for product codes
            # Challenge 5.1, even digit codes
            #
            if run_choice ==5:
                for i in range(4):
                    codes = Stream([random.randint(100000,999999) for j in range(5)])\
                        .source()\
                        .sort()\
                        .map(lambda i: f'X{i}-{sum(list(map(int, str(i)))) % 10}') \
                        .get()
                    print(f'batch {i+1}: {codes}')
            if run_choice == 5.1:
                for i in range(1, 5):
